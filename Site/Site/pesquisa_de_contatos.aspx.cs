...
namespace Site
{
    public partial class pesquisa_de_contatos : _Herdar
    {
        ...
        {
            List<Agenda> Lst = Agenda.Pesquisar(Logado().Id);
        ...
        protected void rptResultado_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            try
            {
                ...
                {
                    case "Alterar":
                        Response.Redirect("~/CadastroDeContato?" + e.CommandArgument);
                        break;
                    case "Remover":
                        if (!AcessoTela().Excluir) throw new Exception("Atenção! Você <b>NÃO</b> tem permissão.");
                        int i = int.Parse(e.CommandArgument.ToString());
                        Agenda o = new Agenda(i);
                        o.Excluir();
                        Type tipo = o.GetType();
                        GravarLog(3, " O CONTATO CODIGO " + o.Id, null, Util.serializarObjetos(o, tipo), tipo);
                        CarregarLista();
                        break;
                }
            }
        ...
        {
            Response.Redirect("~/CadastroDeContato");
        }
    }
}