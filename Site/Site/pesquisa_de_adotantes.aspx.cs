...
namespace Site
{
    public partial class pesquisa_de_adotantes : _Herdar
    {
        ...
        {
            List<Adotante> Lst = Adotante.Pesquisar();
        ...
        protected void rptResultado_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            try
            {
                ...
                {
                    case "Alterar":
                        Response.Redirect("~/CadastroDeAdotante?" + e.CommandArgument);
                        break;
                    case "Remover":
                        if (!AcessoTela().Excluir) throw new Exception("Atenção! Você <b>NÃO</b> tem permissão.");
                        Adotante o = new Adotante(long.Parse(e.CommandArgument.ToString()));
                        o.Excluir();
                        Type tipo = o.GetType();
                        GravarLog(3, " O ADOTANTE CODIGO " + o.Id, null, Util.serializarObjetos(o, tipo), tipo);
                        AtualizaLista();
                        break;
                }
            }
