...
namespace Site
{
    public partial class pesquisa_de_clinicas : _Herdar
    {
        ...
        {
            List<LaboratorioClinico> Lst = LaboratorioClinico.Pesquisar();
        ...
        protected void rptResultado_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            try
            {
                ...
                {
                    case "Alterar":
                        Response.Redirect("~/CadastroDeClinica?" + e.CommandArgument);
                        break;
                    case "Remover":
                        if (!AcessoTela().Excluir) throw new Exception("Atenção! Você <b>NÃO</b> tem permissão.");
                        long l = long.Parse(e.CommandArgument.ToString());
                        LaboratorioClinico o = new LaboratorioClinico(l);
                        o.Excluir();
                        Type tipo = o.GetType();
                        GravarLog(3, " A CLINICA CODIGO " + o.Id, null, Util.serializarObjetos(o, tipo), tipo);
                        CarregarLista();
                        break;
                }
            }
        ...
        {
            Response.Redirect("~/CadastroDeClinica");
        }
    }
}