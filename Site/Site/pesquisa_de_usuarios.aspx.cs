...
namespace Site
{
    public partial class pesquisa_de_usuarios : _Herdar
    {
        ...
        {
            List<Usuario> Lst = Usuario.Pesquisar();
        ...
        protected void rptResultado_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            try
            {
                ...
                {
                    case "Alterar":
                        Response.Redirect("~/CadastroDeUsuario?" + e.CommandArgument);
                        break;
                    case "Remover":
                        if (!AcessoTela().Excluir) throw new Exception("Atenção! Você <b>NÃO</b> tem permissão.");
                        long l = long.Parse(e.CommandArgument.ToString());
                        Usuario o = new Usuario(l);
                        o.Excluir();
                        Type tipo = o.GetType();
                        GravarLog(3, " O USUARIO CODIGO " + o.Id, null, Util.serializarObjetos(o, tipo), tipo);
                        CarregarLista();
                        break;
                }
            }
        ...
        {
            Response.Redirect("~/CadastroDeUsuario");
        }
    }
}