...
namespace Site
{
    public partial class pesquisa_de_colaboradores : _Herdar
    {
        ...
        {
            List<Colaborador> Lst = Colaborador.Pesquisar();
        ...
        protected void rptResultado_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            try
            {
                ...
                {
                    case "Alterar":
                        Response.Redirect("~/CadastroDeColaborador?" + e.CommandArgument);
                        break;
                    case "Remover":
                        if (!AcessoTela().Excluir) throw new Exception("Atenção! Você <b>NÃO</b> tem permissão.");
                        long l = long.Parse(e.CommandArgument.ToString());
                        Colaborador o = new Colaborador(l);
                        o.Excluir();
                        Type tipo = o.GetType();
                        GravarLog(3, " O COLABORADOR CODIGO " + o.Id, null, Util.serializarObjetos(o, tipo), tipo);
                        CarregarLista();
                        break;
                }
            }
        ...
        {
            Response.Redirect("~/CadastroDeColaborador");
        }
    }
}