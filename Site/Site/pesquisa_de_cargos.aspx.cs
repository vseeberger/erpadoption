...
namespace Site
{
    public partial class pesquisa_de_cargos : _Herdar
    {
        ...
        {
            List<Cargo> Lst = Cargo.Pesquisar();
        ...
        protected void rptResultado_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            try
            {
                ...
                {
                    case "Alterar":
                        Response.Redirect("~/CadastroDeCargo?" + e.CommandArgument);
                        break;
                    case "Remover":
                        if (!AcessoTela().Excluir) throw new Exception("Atenção! Você <b>NÃO</b> tem permissão.");
                        long i = long.Parse(e.CommandArgument.ToString());
                        Cargo _item = new Cargo(i);
                        _item.Excluir();
                        GravarLog(3, " O CARGO, CÓDIGO" + i, null, Util.serializarObjetos(_item, _item.GetType()), _item.GetType());
                        CarregarLista();
                        break;
                }
            }
        ...
        {
            Response.Redirect("~/CadastroDeCargo");
        }
    }
}