...
namespace Site
{
    public partial class pesquisa_de_perfis : _Herdar
    {
        ...
        {
            List<Perfil> Lst = Perfil.Pesquisar();
        ...
        protected void rptResultado_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            try
            {
                ...
                {
                    case "Alterar":
                        Response.Redirect("~/CadastroDePerfil?" + e.CommandArgument);
                        break;
                    case "Remover":
                        if (!AcessoTela().Excluir) throw new Exception("Atenção! Você <b>NÃO</b> tem permissão.");
                        int i = int.Parse(e.CommandArgument.ToString());
                        Perfil o = new Perfil(i);
                        o.Excluir();
                        Type tipo = o.GetType();
                        GravarLog(3, " O PERFIL CODIGO " + o.Id, null, Util.serializarObjetos(o, tipo), tipo);
                        CarregarLista();
                        break;
                }
            }
        ...
        {
            Response.Redirect("~/CadastroDePerfil");
        }
    }
}